...
pub enum EndpointMessage {
    ...
    Connected(SocketAddr, Receiver<Vec<u8>>, Sender<Vec<u8>>),
}

impl Debug for EndpointMessage {
    fn fmt(&self, f: &mut Formatter) -> FmtResult {
        match self {
            EndpointMessage::Connect(ref addr) => write!(f, "start connect: {:?}", addr),
            EndpointMessage::Connected(ref addr, _, _) => write!(f, "connected: {:?}", addr),
            EndpointMessage::Disconnect(ref addr) => write!(f, "disconnected: {:?}", addr),
        }
    }
}
